 ...
 *   6. Barres de compétences (IntersectionObserver)
 *   7. Galerie — ruban infini
 *   8. Navigation fluide
 */
...
document.addEventListener('DOMContentLoaded', () => {
  ...
  /* ════════════════════════════════
     ...
     Désactivé sur touch (@media hover:none en CSS)
     ════════════════════════════════ */
  const cursor = document.getElementById('cursor');

  // On vérifie que le dispositif supporte hover (= pas un écran tactile)
  ...
  if (cursor && hasHover) {
    ...
    // Suivi direct (sans lerp) pour une réactivité maximale
    ...
    function animateCursor() {
      // Léger lissage : lerp factor 0.18
    ...
    // État : cartes projet (hover plus grand, mode multiply)
  ...
  if (ribbon) {
    ...
    [...galleryData, ...galleryData].forEach(item => ribbon.appendChild(makeCard(item)));
  }
